...
- the scrobbler gets the first artist listed in metadata by splitting the artist string up across several separators and creating a slice consisting of each section - including the separators - then iterates over that slice, concatenating it together, checking that against musicbrainz's database, then dropping the end off and doing it again until it either finds an artist or reaches the beginning of the string. After finding the artist it logs them to a tab-separated key:value pair, with the metadata string followed by the artist it should send to last.fm. you can easily edit this if you want to make tweaks, or my beautiful flawless system has somehow made a mistake.
